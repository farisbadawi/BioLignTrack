...
export default function TrayScreen() {
  const { patient, trayChanges, dailyLogs, logTrayChange, updateProfile, loadPatientData, hasTreatment } = usePatientStore();
  ...
  // Linked patients without treatment - show waiting message
  if (!hasTreatment && needsSetup) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: themeColors.surface }]} edges={['top', 'left', 'right']}>
        <ScrollView style={[styles.scrollView, { backgroundColor: themeColors.surface }]} contentContainerStyle={styles.scrollContent}>
          ...
          <Card style={{ alignItems: 'center', paddingVertical: Spacing.xl }}>
            <Clock size={48} color={themeColors.textSecondary} />
            <Text style={{ fontSize: 18, fontWeight: '600', color: themeColors.textPrimary, marginTop: Spacing.md }}>
              Waiting for Treatment Plan
            </Text>
            <Text style={{ fontSize: 14, color: themeColors.textSecondary, textAlign: 'center', marginTop: Spacing.xs, paddingHorizontal: Spacing.md }}>
              Your doctor hasn't set up your aligner treatment yet. Once they assign your treatment plan, you'll be able to track your aligners here.
            </Text>
          </Card>
        </ScrollView>
      </SafeAreaView>
